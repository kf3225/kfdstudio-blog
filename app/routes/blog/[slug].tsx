...
export default createRoute(async (c) => {
  ...
  // @ts-ignore
  const modules = import.meta.glob<{ frontmatter: Meta; default: any }>(
    "./posts/*.mdx",
    { eager: true }
  );
  ...
  return c.render(
    <article class="container mx-auto px-4 py-8 max-w-3xl">
      <a
        href="/"
        class="inline-block mb-6 text-blue-600 hover:text-blue-700 transition-colors"
      >
        ← 記事一覧に戻る
      </a>
      <header class="mb-8">
        ...
        <div class="flex items-center gap-4 text-sm text-gray-500">
          <time>
            {new Date(module.frontmatter.pubDate).toLocaleDateString("ja-JP")}
          </time>
          {module.frontmatter.updatedDate && (
            <>
              ...
              <time>
                更新:
                {new Date(module.frontmatter.updatedDate).toLocaleDateString(
                  "ja-JP"
                )}
              </time>
            </>
